...
#include "../libfvde/libfvde_encryption_context.h"

#if defined( __GNUC__ ) && !defined( LIBFVDE_DLL_IMPORT )

/* Tests the libfvde_encryption_context_initialize function
 * Returns 1 if successful or 0 if not
 */
int fvde_test_encryption_context_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libfvde_encryption_context_initialize(
	          &encryption_context,
	...
	result = libfvde_encryption_context_free(
	          &encryption_context,
	          &error );
	...
	encryption_context = (libfvde_encryption_context_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfvde_encryption_context_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfvde_encryption_context_initialize with memset failing
		 */
...
/* Tests the libfvde_encryption_context_free function
 * Returns 1 if successful or 0 if not
 */
int fvde_test_encryption_context_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FVDE_TEST_RUN(
	 "libfvde_encryption_context_initialize",
	 fvde_test_encryption_context_initialize );

	FVDE_TEST_RUN(
	 "libfvde_encryption_context_free",
	 fvde_test_encryption_context_free );

	/* TODO: add tests for libfvde_encryption_context_set_keys */

	/* TODO: add tests for libfvde_encryption_context_crypt */

	/* TODO: add tests for libfvde_encryption_aes_key_unwrap */
