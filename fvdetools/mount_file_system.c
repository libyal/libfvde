	...
	*file_system = memory_allocate_structure(
	     ...
	     error ) != 1 )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to initialize volumes array.",
		 function );
...
/* Appends a volume to the file system
 * Returns 1 if successful or -1 on error
 */
     ...
     libcerror_error_t **error )
{
	     ...
	     error ) != 1 )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to append volume to array.",
		 function );
...
/* Retrieves the path from a volume index.
 * Returns 1 if successful or -1 on error
 */
int mount_file_system_get_path_from_volume_index(
     ...
     libcerror_error_t **error )
{
	static char *function     = "mount_file_system_get_path_from_volume_index";
	size_t path_index         = 0;
	...
	while( volume_number > 0 )
	{
		path[ path_index-- ] = (system_character_t) '0' + ( volume_number % 10 );
